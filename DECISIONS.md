...
## 2026-08-07 — [build] Painéis sob demanda, com o HTML offline como restrição
**Problema:** o app inteiro cabia num chunk só de 670,55 kB (194,22 kB gzip). Quem abria a ÁREA DO CLIENTE baixava os 18 painéis administrativos — conciliação, acessos, modelo, aprovações — para ver um HUD. O aviso do Vite sobre chunk > 500 kB estava lá desde sempre, ignorado.
**Opções:** A) `manualChunks` agrupando vendor/app; B) `React.lazy` no mapa `PAINEIS` do `App.tsx`; C) rota de verdade (router) com split por rota.
**Decisão:** B.
**Por quê:** A separa dependência de código próprio mas não resolve o problema real — o cliente continuaria baixando os painéis. C é a resposta certa a longo prazo, mas o app não tem router (a aba é estado + localStorage) e trocar isso agora é mudança de arquitetura, não de build. B usa a costura que já existe: `PAINEIS` é um `Record<Aba, ComponentType>`, um ponto único, 18 entradas.
**A restrição que dita o desenho:** o HTML offline da apresentação é UM arquivo — o exportador embute os `script[src]` do index.html e, aberto por `file://`, não busca chunk nenhum. Isso continua válido porque o bundler mantém no chunk do entry tudo alcançável ESTATICAMENTE a partir dele, e o `Slideshow` é import estático do `App`. Confirmado no build: `ProjecaoPanel`, `IndicadoresPanel` e os `Relatorio*` NÃO viraram chunk — ficaram no entry, exatamente porque `ConteudoSecao` os alcança. A linha vermelha é introduzir `import()` dentro desse grafo; `tsc` não distingue import estático de dinâmico e o build passa limpo dos dois jeitos, então quem guarda isso é `offlineAutossuficiente.test.ts` (verificado plantando a regressão: o teste falha apontando o arquivo).
**Consequências:** entry 670,55 → 428,53 kB (gzip 194,22 → 130,07 kB, −33%); 26 chunks sob demanda. Um `Suspense` POR painel, não um em volta da lista — com um só, abrir uma aba nova suspenderia o grupo inteiro e trocaria as já visitadas pelo fallback, matando o "voltar é instantâneo" que manter as abas montadas existe para dar. O fallback é uma linha de texto, não o `Carregando` (splash de boot com `min-h-screen` e fundo de login: dentro do `<main>` pareceria que o app reiniciou). `hud` segue estático porque também é a tela inteira nos modos cliente/kiosk. O nome do export é passado como string, então o helper é genérico em `K extends keyof M` — sem isso um typo só apareceria em runtime, naquela aba (verificado: `tsc` recusa `'IncioPanel'`).
**Em entrevista (30s):** "Split de bundle é receita de manual, mas aqui tinha uma amarra: o app exporta um HTML offline autossuficiente pro cliente, e chunk sob demanda não existe em file://. O que salva é a garantia do bundler de que o grafo estático do entry viaja junto — confirmei no build que os componentes da apresentação continuaram no entry. Como nem o compilador nem o build detectam a regressão, escrevi um teste que varre o grafo da apresentação atrás de import dinâmico, e provei que ele falha plantando um."
