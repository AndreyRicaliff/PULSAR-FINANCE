...
## 2026-08-07 — [apresentacao] Guard de sobrescrita por CONTEÚDO, não por id
**Problema:** a equipe financeira perdeu fechamentos. "Ao sair e voltar pra editar, as alterações somem, em algumas empresas." Há duas memórias para um documento: o roteiro em edição (`painel_estado`, chave `cliente:<id>:apresentacao-v2`, UM por empresa) e a apresentação salva (`painel_apresentacoes`). Abrir uma apresentação copia linha → roteiro, destrutivamente. O guard dessa cópia perguntava "é outro card?" (`abertaId !== item.id`) e por isso calava nos dois caminhos mais usados: reabrir o MESMO card, e voltar à aba depois do remount (aí `abertaId` já é null e o `confirmarTroca` se auto-aprovava em `if (!abertaId) return true`). Evidência em prod: 3 das 6 empresas com roteiro salvo estavam no tamanho exato do padrão vazio — 10 slides, ZERO texto (HOPE PIZZAS 974 B, DR PIZZA MALVINAS 1009 B, AUTO-PEÇAS SERTANEJA 964 B).
**Opções:** A) autosalvar a linha da apresentação a cada tecla, eliminando a divergência; B) guardar em qual linha o roteiro foi aberto e comparar por id persistido; C) comparar CONTEÚDO — assinatura do roteiro em edição contra o das apresentações da empresa.
**Decisão:** C, com indicador visível de "alterações não salvas" no cabeçalho do editor.
**Por quê:** A transforma "Salvar" em enfeite e cria uma classe nova de divergência entre rascunho e publicada; B precisa de persistência extra só para responder uma pergunta que o próprio conteúdo já responde, e volta a depender de um identificador que o remount perde — exatamente a fragilidade que causou o bug. C não guarda estado nenhum: os dois lados são carregados a cada render e a comparação é sobre o que o analista escreveu. Assinatura EXPLÍCITA campo a campo, nunca `JSON.stringify` do objeto cru — os lados vêm de origens diferentes (spread em memória × `JSON.parse` do Postgres) e a ordem das chaves acusaria diferença onde não há, gerando confirm a cada clique até ninguém mais ler o aviso.
**Consequências:** o confirm agora aparece quando o roteiro em edição não bate com NENHUMA apresentação da empresa — inclusive na primeira abertura de um tenant cujo buffer ficou órfão (é verdade, não ruído: aquele conteúdo não está salvo em lugar nenhum). Roteiro pristino nunca alarma, porque o padrão já nasce com 10 slides e 4 títulos e "tem texto" não serve de teste — o que caracteriza trabalho é DIFERIR do padrão. `normalizar` saiu de `useApresentacao` para `lib/rascunhoApresentacao` (o explorador precisa dele fora do hook).
**Em entrevista (30s):** "O guard existia, mas comparava qual card estava aberto. Só que esse id zera quando o componente remonta, e reabrir o mesmo card pulava a comparação inteira — a proteção calava justamente nos dois caminhos que as pessoas usam. Troquei identidade por assinatura de conteúdo: a pergunta certa não é 'é outro arquivo?', é 'eu tenho trabalho que não está salvo em lugar nenhum?'. Achei a extensão do estrago com uma query em produção: metade das empresas estava com o roteiro no tamanho exato do template vazio."

## 2026-08-07 — [persistencia] O remoto velho não apaga mais a cópia local nova
**Problema:** no boot, `obterStore` hidratava do remoto sempre que `remoto != null && !s.sujo` — e `sujo` nasce `false` a cada reload. Quem digitasse e fechasse a aba dentro dos 600 ms de debounce, ou tivesse o upsert recusado, tinha a cópia do localStorage (a MAIS NOVA) descartada em silêncio pela versão velha do servidor. O `console.error` do upsert recusado não chega a ninguém.
**Opções:** A) encurtar o debounce; B) marcar as chaves com gravação remota não confirmada e, no boot, preferir a local e reenviá-la; C) versionar o documento e resolver por timestamp.
**Decisão:** B + descarga do debounce em `visibilitychange`/`pagehide`.
**Por quê:** A só reduz a janela e aumenta o tráfego. C exige um relógio confiável dos dois lados e `painel_estado.atualizado_em` **não serve**: a tabela não tem trigger e o app faz upsert sem essa coluna, então o valor é a data de CRIAÇÃO da chave, não do último save (verificado em prod — AUTAG 27 marcava 15/06 com conteúdo de julho). B usa a informação que já existe localmente.
**Consequências:** a marca só é gravada quando o `setItem` REALMENTE funcionou. Sem esse cuidado o fix inverteria o bug em tenant grande: `movimentos-raw` passa de 16 MB numa única empresa em prod (95 MB somados nas 24), o localStorage estoura a cota, a cópia local fica defasada e o boot reenviaria lixo por cima do bom. O timer passa a ser zerado ao disparar — `descarregarPendentes` o usa como "há algo por gravar" e sem isso toda troca de aba reenviava gravações já concluídas.
**Em entrevista (30s):** "O app tinha backup local e mesmo assim perdia dados, o que é o pior dos mundos: no boot ele jogava fora o backup e ficava com a versão velha do servidor. Marquei quais chaves têm gravação não confirmada para o local ganhar e ser reenviado. A parte não-óbvia foi só marcar quando a escrita local deu certo — tem cliente com 16 MB de movimentos, o localStorage estoura, e a versão ingênua do fix passaria a sobrescrever o servidor bom com um cache velho."
