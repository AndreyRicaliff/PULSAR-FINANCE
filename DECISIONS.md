...
## 2026-08-07 — [sync-omie] Bug de auditoria refutado com payload real; edge ganha módulo puro testável
**Problema:** auditoria multi-agente apontou 2 bugs na edge: (G) o rateio `s.categories[]` "não traria" `type` → `str(undefined)=''` → arquivadas NIBO caindo em natureza `outra`; (F) guarda-de-zero só nos movimentos — throttle no `ListarCategorias` gravava `categorias:[]` por cima do espelho bom. O fix G proposto era trocar `c.type` por `s.type`.
**Opções:** A) aplicar o fix G como veio; B) validar o shape real com apitoken antes (rito PLANO_NIBO §2); C) só o fix F.
**Decisão:** B — a sonda refutou G: o rateio TRAZ `type` próprio (`'in'`/`'out'`), prod tem ZERO `outra` nos 16 tenants NIBO e 7 arquivadas do tenant sondado são recuperadas corretas hoje. `c.type` fica, com fallback defensivo `|| s.type`; F confirmado e corrigido; funções puras extraídas para `categorias-nibo.ts` com testes fixando o shape real.
**Por quê:** trocar para `s.type` às cegas seria regressão latente — rateio pode carregar sinal próprio ≠ schedule (estorno/desconto). A auditoria errou porque o PLANO_NIBO.md omitia o campo `type` do rateio (doc corrigida no mesmo commit). Extração em vez de gêmeo em `src`: o canônico É a edge; gêmeo de teste deriva em silêncio (o padrão gêmeo do merge/`sincronizacao.ts` vale quando o canônico vive no `src`).
**Consequências:** a edge deixa de ser single-file (deploy sobe 2 arquivos; `deno check` valida a resolução); `obj`/`str` ganham gêmeos de 1 linha no módulo; achado de auditoria estática sobre shape de payload externo nunca mais vira fix sem sonda no dado real.
**Em entrevista (30s):** "A auditoria tinha fix pronto e mecanismo plausível: campo ausente virava default silencioso. Antes de aplicar, puxei o payload real com a credencial do tenant — o campo existia, o bug não. Apliquei só o que sobreviveu ao dado: o outro bug (confirmado), um fallback defensivo, testes que fixam o shape real e a correção da doc que tinha enganado a auditoria."
