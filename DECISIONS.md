...
## 2026-08-07 — [dados] Protocolo de datas e identidade: nomear a proveniência em vez de mudar a âncora
**Problema:** "registros mudando de mês, sem data ou sem código" (equipe, 07/08). Medição em prod: zero registros sem data — mas em 4 tenants Omie 94–99,9% dos movimentos são eventos de extrato SEM emissão, ancorados em competência pela data de PAGAMENTO (fallback da canônica). A DRE desses clientes é caixa com outro rótulo, e `coberturaDatas` respondia "100% com data". E o "sem código": 68.696 eventos de extrato têm idTitulo '0' com idMovCC presente em 100% — mas dois consumidores ignoravam isso (override de título via `idTitulo || documento`, onde '0' é truthy → renomear UM evento renomeava TODOS; e `mesclarMovimentos` mapeando por idTitulo cru → colapsaria tudo numa entrada).
**Opções:** A) mudar a âncora (extrato fora da competência, ou sempre caixa); B) manter a âncora e NOMEAR a proveniência, avisando quando o mês lido está ancorado pelo caixa; C) só documentar.
**Decisão:** B + identidade canônica única + glossário de origens.
**Por quê:** A muda TODO número de 4 clientes sem decisão de negócio — e o comportamento atual é uma escolha consciente (evento sem emissão não pode sumir da série). C não muda o que a equipe vê. B é o meio honesto: `ancoraDoMovimento` devolve `{iso, fonte, emprestada}` com `iso` provado byte-idêntico ao histórico por força bruta (32 combinações × 2 regimes contra a fórmula antiga literal); o aviso do período passa a mostrar "N (x%) ancorados pela data de pagamento". Identidade: `chaveMovimento` promove a chave que `chaveMovFilial` JÁ persistia (cc:→t:→x:, bytes preservados — mudar a ordem órfãria mapas de centros salvos); prod tinha 0 overrides de título, então a troca da chave no override não órfã nada. Origens: catálogo `core/origens.ts` com censo real (EXTR 43.627 · EXTP 22.501 · NIBO 55.687), tooltip no detalhamento e doc para a equipe (docs/DATAS_E_IDENTIDADE.md).
**Consequências:** nenhum número muda (o contrato central); a mina do merge está desarmada ANTES do primeiro chamador; TituloCelula ganha edição mesmo sem rótulo (apelidar extrato sem documento é caso real); decidir SE a âncora deve mudar (extrato em competência = vencimento? fora?) virou decisão de produto informada pelo aviso, não default silencioso.
**Em entrevista (30s):** "O sintoma era 'registro mudando de mês'. Medi produção antes de codar: não existia registro sem data — existiam 4 clientes onde 99% dos lançamentos eram extrato bancário sem emissão, ancorados pela data de pagamento numa DRE que dizia competência. Não mudei a âncora: mudar número de cliente é decisão de negócio. Nomeei a proveniência — cada âncora diz de onde veio — provei por força bruta que nenhum número mudou, e o aviso agora conta o empréstimo. O 'sem código' era o app ignorando a identidade que o dado já tinha: 100% dos eventos de extrato têm id de lançamento bancário."
