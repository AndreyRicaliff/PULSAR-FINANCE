...
## 2026-08-07 — [visual] Revisão v4 aplicada com a régua da casa, não com os números do pacote
**Problema:** handoff do Claude Design (revisão visual completa: claro apagado, escuro assentado, navegação que se revela, vocabulário de movimento). O arquivo com os valores exatos (patch de 10 blocos) veio corrompido no download — e, pior, os contrastes ALEGADOS no resumo estavam errados: warn claro "4,6" media 3,35; muted claro "4,9" media 4,33 no bg. Aplicar de olho fechado colocaria o tema claro inteiro fora de norma.
**Decisão:** aplicar as 8 etapas na ordem do handoff, mas TODO token passa pela régua WCAG própria antes de entrar; os 3 reprovados foram recalculados preservando matiz (warn 866B2F, muted 736D8F, danger B6524A). Valores ausentes (mapa de duração por utility) derivados do vocabulário declarado (120/240/480/700, um easing).
**Desvios declarados:** (1) três tokens diferem do pacote — motivo acima; (2) sidebar comprimida chegou a 914px de nav, não os 900 do checklist — zero-scroll custaria a densidade confortável que é o objetivo maior; (3) mk-dot de "novidades não lidas" segue pulsando em repouso — é estado REAL, não decoração; (4) varredura bold→semibold "só em telas claras" é impraticável com classes Tailwind theme-agnósticas — aplicado apenas onde o peso é CSS próprio (mk-kpi__valor 800→700).
**Verificação:** contrastes medidos por script; app exercitado no navegador — tokens computados conferidos nos DOIS temas, seções renomeadas, busca/chips/avatar presentes, `?static=1` com zero animação rodando e zero elemento invisível, únicas animações infinitas em repouso são a marca viva e o dot de novidades.
**Em entrevista (30s):** "Recebi um handoff de design com os contrastes 'medidos'. Medi de novo — dois reprovavam. A lição não é que o gerador errou: é que contraste alegado não é contraste medido, e o pipeline precisa da régua no ponto de entrada. Apliquei a revisão inteira com os valores corrigidos e declarei cada desvio no PR."
