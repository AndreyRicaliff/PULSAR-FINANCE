...
## 2026-08-06 — [apresentacao] Versão atribuída no banco + export não sai zerado calado
**Problema:** dois reports juntos — (1) "erro de duplicidade que não existe" ao criar/salvar apresentação; (2) "apresentações salvando em html em branco".
**Diagnóstico (reproduzido em prod, com rollback):** (1) a unique é `(cliente_id, competencia, versao)`, o front nunca informava `versao` (default 1) e `competencia` nascia sempre no mês corrente — logo a SEGUNDA apresentação de qualquer cliente no mesmo mês colidia, mesmo com título diferente; (2) o HTML gerava certo, mas saía com R$ 0,00 em tudo por duas causas independentes: cliente sem Matriz conciliada (9 dos 23 tenants) e período da apresentação sem movimento.
**Opções:** A) front lê o max(versao) e insere versao+1; B) trigger BEFORE INSERT numera no banco; C) largar a unique. Para o vazio: D) bloquear o export; E) avisar na tela + confirmação explícita; F) deixar como está.
**Decisão:** B + E.
**Por quê:** A custa um round-trip e ainda corre entre duas abas; C perderia o modelo de versões (v1/v2 do mesmo fechamento é domínio real do BPO) e deixaria a unique só como enfeite. O `max()+1` roda dentro da transação do INSERT e a unique permanece como backstop. Para o vazio, D seria errado: existe caso legítimo (mês fechado sem operação) — mas ninguém pode entregar R$ 0,00 ao cliente por descuido, então a confirmação é obrigatória e o motivo é nomeado ("concilie a Matriz" × "ajuste o período"), na ordem do que o operador resolve primeiro.
**Consequências:** competência passa a ser derivada do PERÍODO da apresentação (fechamento de julho montado em agosto deixa de nascer carimbado 'ago'); insert grava o roteiro padrão em vez de `{}`; criar apresentação nova pede a mesma confirmação que abrir; 23505 residual (corrida de abas) vira frase humana. Núcleo do diagnóstico é puro e testado (`core/diagnosticoApresentacao.ts`).
**Em entrevista (30s):** "O erro dizia duplicidade e o usuário jurava que não havia — os dois estavam certos: a chave única era (cliente, competência, versão) e ninguém preenchia a versão. Numerar no cliente parece simples mas corre; pus no banco, dentro da transação do insert, com a unique como rede. O segundo bug era pior porque era silencioso: o relatório saía zerado e ia pro cliente. Diagnostiquei ANTES de exportar e nomeei a causa que o operador consegue resolver."

## 2026-08-06 — [apresentacao] Imagens da marca embutidas no HTML offline
**Problema:** o HTML exportado pedia `/pulsar-finance-wordmark.png` pela REDE — 404 garantido num arquivo aberto por `file://` (o caminho absoluto resolve na raiz do disco). A marca sumia do relatório entregue ao cliente. Achado pela aba de rede do próprio artefato de produção, não por relato.
**Opções:** A) inlinar as `<img>` no documento, como já é feito com `<script src>` e `<link rel=stylesheet>`; B) transportar as imagens no snapshot como data: URI e resolvê-las em runtime; C) caminho relativo em vez de absoluto.
**Decisão:** B — `SnapshotApresentacao.assets` (caminho → data: URI) + `urlAsset()` no ponto de uso.
**Por quê:** A não funciona: as `<img>` nascem em RUNTIME (React), e o inliner opera sobre o index.html estático, onde elas não existem. C troca um 404 por outro — o HTML fica solto na pasta de Downloads do cliente, sem os assets ao lado. Em B o arquivo continua verdadeiramente autossuficiente, que é a promessa do formato.
**Consequências:** `urlAsset` degrada para o caminho original fora do modo Apresentação e para asset que o export não alcançou (`<img src>` nunca fica `undefined`); asset ausente vira `console.warn`, não derruba o export; +66 kB base64 num arquivo de ~6 MB. Imagem nova usada por `<img>` precisa entrar em `ASSETS_DA_MARCA` — o teste de regressão documenta isso.
**Em entrevista (30s):** "O relatório chegava ao cliente sem a marca e ninguém tinha reportado — achei olhando a aba de rede do arquivo real, não o código. A correção óbvia (inlinar a tag) não funcionaria, porque a tag não existe no HTML de origem: ela nasce em runtime. Então transportei o binário junto com os dados, como data URI, e resolvi no ponto de uso — com fallback para o caminho original, para o app normal não mudar em nada."

## 2026-08-06 — [hud] A resposta antes da ferramenta, e a língua do dono
**Problema:** pedido "embelezar a área do cliente". O que estava feio não era o CSS: o cliente abria e a PRIMEIRA coisa na tela era o painel de filtros do contador ("Competência × Caixa", "0% têm data no regime competencia"). Ele via a ferramenta do BPO, não um painel dele — e para saber "como foi o mês" precisava interpretar KPIs.
**Opções:** A) polir o visual (sombra, gradiente, ícone) mantendo a estrutura; B) inverter a ordem — manchete primeiro, ferramenta recolhida — e traduzir o vocabulário; C) construir uma tela nova só do cliente, separada do pipeline dos relatórios.
**Decisão:** B, com o polimento (count-up, stagger) vindo de carona na manchete nova.
**Por quê:** A trata o sintoma — a tela continuaria pedindo que o dono da pizzaria soubesse o que é regime de competência. C duplicaria o motor de cálculo, e a lição do app inteiro é ter UMA fonte de número (o HUD sempre delegou a `useResultado`). B mantém a fonte única e muda só a ORDEM e a LÍNGUA: os mesmos componentes falam duas línguas conforme `useSomenteLeitura`.
**Consequências:** `core/manchete.ts` puro e testado decide o que é dizível — em especial descartar o mês corrente (mês incompleto anunciado como fechado é dado verdadeiro comunicando mentira) e não emitir percentual sobre base zero. `FiltroPeriodo` e `IndicadoresPanel` passam a ter dois vocabulários; o custo é lembrar dos dois ao editar, e o teste trava a ausência de jargão no lado do cliente. A marca do HUD segue sendo a da AG — decisão pendente do Ricaliff.
**Em entrevista (30s):** "Pediram para embelezar e o problema não era estético: a tela abria pelo filtro do contador, com 'regime de competência' na cara de quem só quer saber se sobrou dinheiro. Inverti a ordem — uma frase em português responde primeiro, a ferramenta recolhe — e fiz os mesmos componentes falarem duas línguas conforme o papel de quem lê. A regra que mais importa virou teste: a manchete só fala de mês FECHADO, porque anunciar mês pela metade é um número verdadeiro contando uma mentira."

## 2026-08-06 — [apresentacao] O congelamento era só texto; e a marca do cliente no painel dele
**Problema:** pergunta "a apresentação passa da conta adm pra conta do cliente, em tempo real?". Ao verificar: (a) existia Publicar e a RLS que autoriza o cliente a ler `publicada`, mas NENHUMA tela no lado dele — as 8 referências a `painel_apresentacoes` estavam todas no explorador do operador; (b) pior, o publish gravava `numeros: {}` (2 bytes nas 6 linhas de prod), então o "o conteúdo congela" da confirmação era falso — reclassificar em setembro mudaria o julho já entregue.
**Opções:** A) abrir a tela do cliente sobre o pipeline vivo (`useResultado`); B) congelar de verdade no publish e a tela do cliente ler a foto; C) só listar e deixar o cliente baixar o HTML exportado.
**Decisão:** B. E a marca: logo do cliente CONVIVE com a assinatura da AG, não substitui.
**Por quê:** A entregaria uma tela que mente — o número mudaria depois de entregue, que é exatamente o que a migration de 24/07 dizia querer impedir. C não resolve: o HTML é gerado no navegador do operador, não existe no banco. Em B a foto sai do MESMO pipeline (totaisEfetivos → calcular), porque duas fórmulas para o mesmo número é como o painel passa a divergir de si; e o intervalo entra por parâmetro, senão publicar julho em agosto congelaria agosto.
**Consequências:** `lerCongelado` devolve null para o `{}` legado — a UI avisa em vez de mostrar zero como fato, e as 6 publicadas antigas continuam abrindo. Não há realtime (nem existe no app): o cliente recarrega ou usa "Procurar novos" — para fechamento mensal, e-mail no publish resolveria melhor que websocket. Logo é data URI (teto 400 kB) porque o painel precisa funcionar dentro do HTML offline, onde link externo não resolve.
**Em entrevista (30s):** "Perguntaram se o relatório chegava ao cliente. Fui verificar e achei duas metades faltando: não havia tela do lado dele, e o congelamento que o botão prometia nunca tinha sido implementado — o banco exigia 'numeros não nulo' e recebia um objeto vazio, que passa no check e não guarda nada. Corrigi na ordem certa: primeiro congelar de verdade, depois abrir a tela. Abrir antes seria entregar uma tela que mente."

## 2026-08-07 — [hud] Manchete empilhada: resultado manda, caixa assina embaixo
**Problema:** decisão do Ricaliff (07/08) sobre a pergunta da manchete: "as duas, empilhadas" — resultado E caixa. O risco de empilhar é diluir o "uma resposta antes da ferramenta" e, pior, a linha de caixa MENTIR com cara de precisão: cobertura depende do saldo bancário, que a Omie não sincroniza (é entrada manual da AG, por mês — a mesma da Projeção).
**Opções para o "sem saldo informado":** A) esconder a linha; B) assumir saldo 0 e calcular cobertura; C) dizer o que sabe (contas a pagar no corte) sem claim de cobertura.
**Decisão:** C, com três regras de honestidade no núcleo (`core/caixaCliente.ts`): (1) "cobre até X" só existe com saldo informado — e `mes in saldos`, nunca `?? 0`, porque saldo ZERO é um saldo real que gera cobertura negativa, enquanto saldo AUSENTE não pode fingir nenhuma; (2) conta vencida e não paga continua devida — entra no corte como devida hoje; (3) o número é operacional e muda todo dia — a UI carimba "sincronizado em DD/MM" em vez de dar cara de eterno.
**Hierarquia visual:** o rodapé é visivelmente secundário — tipografia menor, sem count-up, sem stagger; o palco segue do resultado. A cobertura anda pelos vencimentos ordenados (vencida = hoje) até o caixa acabar: "cobre tudo" / "cobre até 28 de agosto" / "não cobre a próxima conta".
**Consequências:** a linha some quando não há nada útil a dizer (null), aparece informativa sem saldo, e completa com saldo; a receber é contexto e NUNCA entra na cobertura (recebível previsto não paga a conta de amanhã). Fonte = TODOS os movimentos (`useMovimentos`), não o recorte do período — título a vencer semana que vem está fora de qualquer período passado.
**Em entrevista (30s):** "O dono quer duas respostas: 'deu certo?' e 'posso pagar?'. A segunda depende do saldo bancário, que o ERP não fornece — é manual. A tentação era assumir zero e calcular; só que zero informado e zero por ausência são coisas diferentes: um gera cobertura negativa real, o outro geraria mentira. Sem saldo, a linha diz o que sabe e explica o que falta. E conta vencida continua devida — o corte de 30 dias que esquece a vencida diria 'cobre tudo' pra quem está inadimplente."
