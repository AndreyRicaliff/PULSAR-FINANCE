/**
 * @file Catálogo de relatórios: o que está disponível, parcial ou aguardando dado
 * (honestidade primeiro). `{provedor}`/`{de}`/`{em}` são interpolados no render.
 */
...
function Card({ item, onAbrir }: { item: Item; onAbrir: (v: VistaRel) => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-muted">{comProvedor(item.desc, rotulos)}</p>
      {item.falta ? <p className="text-[11px] text-muted/80">Falta: {comProvedor(item.falta, rotulos)}</p> : null}
