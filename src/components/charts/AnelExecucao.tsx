...
export function AnelExecucao({
  ...
  natureza: 'R' | 'P'
  rotulo: string
  /** Override da cor do arco (ex.: warn p/ manutenção no CAPEX) — o padrão segue a natureza. */
  cor?: string
}) {
  const R = 26
  const CIRC = 2 * Math.PI * R
  const frac = pct === null ? 0 : Math.min(Math.max(pct, 0), 100) / 100
  const cor = corOverride ?? (natureza === 'R' ? 'rgb(var(--c-accent))' : 'rgb(var(--c-danger))')
  return (
